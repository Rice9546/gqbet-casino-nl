...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-bd9bb22c";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-bd9bb22c">'+'<span class="tile-face-bd9bb22c tile-front-bd9bb22c"></span>'+'<span class="tile-face-bd9bb22c tile-back-bd9bb22c"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-bd9bb22c")){return;}
var back=tile.querySelector(".tile-back-bd9bb22c");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-bd9bb22c");if(isMine){tile.classList.add("boom-bd9bb22c");}
if(dim){tile.classList.add("dimmed-bd9bb22c");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-bd9bb22c")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-bd9bb22c");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-bd9bb22c")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-bd9bb22c");void panelEl.offsetWidth;panelEl.classList.add("jolt-bd9bb22c");}
endRound();}
